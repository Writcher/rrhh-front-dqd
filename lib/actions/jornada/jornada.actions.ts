...
export async function getAusenciasPendientesCountForProyectos(): Promise<Record<number, number>> {
    try {
        const token = await getToken();

        const total = await fetch(`${CONFIG.URL_BASE}${CONFIG.URL_JORNADA}/ausencias/pendiente/count/all`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`
            }
